...
function mosseFilter(params) {
    ...
    this.track = function(input, left, top, width, height, updateFilter, gaussianPrior, calcPSR) {
        ...
        // var id = image.data;
        console.error("mosse in:", input.width, input.height, input.videoWidth, input.videoHeight)

        var imgLeft = Math.round((left / input.width) * input.width);
        var imgTop = Math.round((top / input.height) * input.height);
        var imgWidth = Math.round((width / input.width) * input.width);
        var imgHeight = Math.round((height / input.height) * input.height);

        console.error("mosse trim:", imgLeft, imgTop, imgWidth, imgHeight)
        var image = gd.create(_w, _h);
        image.copy(input, 0, 0, imgLeft, imgTop, imgWidth, imgHeight);
        // _cc.drawImage(input, videoLeft, videoTop, videoWidth, videoHeight, 0, 0, _w, _h);
